...
bool ExtrinsicsCalibrator3D::generateHeadFK()
{
  const std::string root_frame = "BASE";
  const std::string tip_frame = "HEAD";
  ...
  chain.initialize(root_frame, tip_frame);
  kdl_joint_array.resize(chain.getNumJoints());
  std::vector<double> joint_positions;
  joint_positions.resize(chain.getNumJoints(), 0.0);
  ROS_INFO("Initialized forward kinematic chain with >%d< joints.", chain.getNumJoints());
  ...
  std::string output_file_name = output_path + "calib/verify_forward_kinematics_output.txt";
  std::ofstream forward_kinematic_file;
  forward_kinematic_file.open(output_file_name.c_str(), std::ios::out);
  if(!forward_kinematic_file.is_open())
  {
    ROS_ERROR("Could not open file >%s< to write out forward kinematic verification values.", output_file_name.c_str());
    return false;
  }
  ...
  while(lower_pan < LOWER_PAN_MAX - increment)
  {
    ...
    while(lower_tilt < LOWER_TILT_MAX - increment)
    {
      ...
      while(upper_pan < UPPER_PAN_MAX - increment)
      {
        ...
        {
          joint_positions[0] = lower_pan;
          joint_positions[1] = lower_tilt;
          joint_positions[2] = upper_pan;
          joint_positions[3] = upper_tilt;

          // write to file
          joint_angle_file << joint_positions[0];
          joint_angle_file <<  " ";
          joint_angle_file << joint_positions[1];
          joint_angle_file <<  " ";
          joint_angle_file << joint_positions[2];
          joint_angle_file <<  " ";
          joint_angle_file << joint_positions[3];
          joint_angle_file <<  "\n";
