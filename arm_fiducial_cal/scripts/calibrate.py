...
yamlfile_path = os.path.join(roslib.packages.get_pkg_dir("arm_fiducial_cal"), "calib/correction_gp.yaml")
...
txtfile_input_path = os.path.join(roslib.packages.get_pkg_dir("arm_fiducial_cal"), "calib/correction_gp_input.txt")
txtfile_input = open(txtfile_input_path, 'w+')
for vec in gp_training_inputs:
    print >> txtfile_input, ''.join(['%7.3f' % x for x in vec]) + ''
txtfile_input.close()

txtfile_output_path = os.path.join(roslib.packages.get_pkg_dir("arm_fiducial_cal"), "calib/correction_gp_output.txt")
txtfile_output = open(txtfile_output_path, 'w+')
for vec in gp_training_outputs:
    print >> txtfile_output, ''.join(['%7.3f' % x for x in vec]) + ''
...
print ''
print 'Head calibration is done. Now learning GP...'
print ''
