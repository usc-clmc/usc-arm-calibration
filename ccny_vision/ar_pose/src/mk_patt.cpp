...
static void Display(void)
{
  ...
  if (gTarget != NULL)
  {
    ...
    lineSeg(
            gTarget->vertex[0][0], gTarget->vertex[0][1], gTarget->vertex[1][0], gTarget->vertex[1][1], gArglSettings,
            gARTCparam, 1.0);
    lineSeg(
            gTarget->vertex[3][0], gTarget->vertex[3][1], gTarget->vertex[0][0], gTarget->vertex[0][1], gArglSettings,
            gARTCparam, 1.0);
    glColor3d(1.0, 0.0, 0.0);
    lineSeg(
            gTarget->vertex[1][0], gTarget->vertex[1][1], gTarget->vertex[2][0], gTarget->vertex[2][1], gArglSettings,
            gARTCparam, 1.0);
    lineSeg(
            gTarget->vertex[2][0], gTarget->vertex[2][1], gTarget->vertex[3][0], gTarget->vertex[3][1], gArglSettings,
            gARTCparam, 1.0);
    endOrtho2D();
  }
...
static bool got_camera_info;
static bool got_camera_image;

void camInfoCallback(const sensor_msgs::CameraInfoConstPtr& cam_info)
{
  if (!got_camera_info)
  {
    sensor_msgs::CameraInfo cam_info_ = (*cam_info);

    gARTCparam.xsize = cam_info_.width;
    gARTCparam.ysize = cam_info_.height;

    gARTCparam.mat[0][0] = cam_info_.P[0];
    gARTCparam.mat[1][0] = cam_info_.P[4];
    gARTCparam.mat[2][0] = cam_info_.P[8];
    gARTCparam.mat[0][1] = cam_info_.P[1];
    gARTCparam.mat[1][1] = cam_info_.P[5];
    gARTCparam.mat[2][1] = cam_info_.P[9];
    gARTCparam.mat[0][2] = cam_info_.P[2];
    gARTCparam.mat[1][2] = cam_info_.P[6];
    gARTCparam.mat[2][2] = cam_info_.P[10];
    gARTCparam.mat[0][3] = cam_info_.P[3];
    gARTCparam.mat[1][3] = cam_info_.P[7];
    gARTCparam.mat[2][3] = cam_info_.P[11];

    gARTCparam.dist_factor[0] = cam_info_.K[2]; // x0 = cX from openCV calibration
    gARTCparam.dist_factor[1] = cam_info_.K[5]; // y0 = cY from openCV calibration
    gARTCparam.dist_factor[2] = -100 * cam_info_.D[0]; // f = -100*k1 from CV. Note, we had to do mm^2 to m^2, hence 10^8->10^2
    gARTCparam.dist_factor[3] = 1.0; // scale factor, should probably be >1, but who cares...
...
void getTransformationCallback(const sensor_msgs::ImageConstPtr& image_msg)
{
  ...
  if (!got_camera_image)
  {
    ...
    try
    {
      capture = bridge.imgMsgToCv(image_msg, "bgr8");
    }
    ...
    cvSaveImage(filename.c_str(), capture);

    //cvConvertImage(capture,capture,CV_CVTIMG_FLIP);
    gARTsaveImage = (ARUint8 *)capture->imageData;
    got_camera_image = true;
  }
}
...
int main(int argc, char *argv[])
{
  ...
  ros::init(argc, argv, "MakeARPatternNode");
  ros::NodeHandle node_handle;
  mutex.lock();
  got_camera_info = false;
  ros::Subscriber sub = node_handle.subscribe("/Honeybee/left/camera_info", 1, camInfoCallback);
  while (ros::ok() && !got_camera_info)
  {
    ros::spinOnce();
  }
  sub.shutdown();
...
//  }
  ...
  got_camera_image = false;
  ROS_INFO("Waiting for image.");
  ros::Subscriber sub2 = node_handle.subscribe("/Honeybee/left/image_rect_color", 1, getTransformationCallback);
  while (ros::ok() && !got_camera_image)
  {
    ros::spinOnce();
  }
  sub2.shutdown();
  ...
  ROS_INFO("Size %i %i ", gARTCparam.xsize, gARTCparam.ysize);
