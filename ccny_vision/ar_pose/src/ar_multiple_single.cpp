...
// local includes
#include <ar_pose/ar_multiple_single.h>
  ...
  ARBase(node_handle)
{
  ...
  // advertise
  ar_marker_publisher_ = node_handle_.advertise<ar_pose::ARMarkers> ("ar_pose_marker", 0);
  if (publish_visual_markers_)
  {
    rviz_marker_pub_ = node_handle_.advertise<visualization_msgs::Marker> ("visualization_marker", 0);
  }
  ar_marker_pub_ = node_handle_.advertise<ar_pose::ARMarkers> ("ar_pose_marker", 0);
}
...
void ARMultipleSinglePublisher::camInfoCallback(const sensor_msgs::CameraInfoConstPtr & cam_info)
{
  if (!get_camera_info_)
  {
    ...
    cam_param_.dist_factor[2] = 0.0; // image is already rectified
...
void ARMultipleSinglePublisher::arInit()
{
  ...
  ROS_INFO("Read >%i< objects.", objectnum_);
...
void ARMultipleSinglePublisher::addMarker(const int index, const tf::Transform& transform)
{
  ...
  ar_pose_marker.id = marker_info_[index].id;
  ar_pose_marker.confidence = marker_info_[index].cf;

  ar_pose_marker.quaternion.w = transform.getRotation().getW();
  ar_pose_marker.quaternion.x = transform.getRotation().getX();
  ar_pose_marker.quaternion.y = transform.getRotation().getY();
  ar_pose_marker.quaternion.z = transform.getRotation().getZ();

  for (int i = 0; i < 4; ++i)
  {
    int vertex_index = (marker_info_[index].dir + i) % 4;
    // ROS_INFO("vertex_index = %i.", vertex_index);
    // ar_pose_marker.u_corners.push_back(marker_info_[index].vertex[vertex_index][0]);
    // ar_pose_marker.v_corners.push_back(marker_info_[index].vertex[vertex_index][1]);
    ar_pose_marker.u_corners.push_back(marker_info_[index].vertex[vertex_index][0]);
    ar_pose_marker.v_corners.push_back(marker_info_[index].vertex[vertex_index][1]);
  }

  ar_markers_.markers.push_back(ar_pose_marker);
}
         ...
         char **argv)
{
  ...
  ar_pose::ARMultipleSinglePublisher ar_multiple_single(node_handle);
  ros::spin();
  return 0;
}
