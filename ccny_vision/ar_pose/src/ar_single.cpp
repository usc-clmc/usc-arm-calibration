    ...
    ARBase(node_handle)
{
  ...
  // advertise
  ar_marker_publisher_ = node_handle_.advertise<ar_pose::ARMarker> ("ar_pose_marker", 0);
...
//  ROS_INFO("AFTER:  origin = >%i<.", capture_->origin);
  ...
  if (k != -1)
  {
    ...
    ar_marker_publisher_.publish(ar_pose_marker_);
         ...
         char **argv)
{
  ...
  ar_pose::ARSinglePublisher ar_single(node_handle);
  ros::spin();
  return 0;
}
