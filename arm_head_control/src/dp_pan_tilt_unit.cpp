...
/****                                                                *****
*****             (C)1992-2010, FLIR Montion Control Systems, Inc.   *****
*****                     All Rights Reserved.                       *****
*****                                                                *****
...
CHANGE HISTORY:
     3/19/07  v2.13.1    Added ISM support to ABS Pos Calls and made all variables signed
     8/17/06  v2.12.13   Added pan trigger interface
     7/25/06             Fixed Timeout Async error.
     2/28/06  v2.12.12r7 Added tilt vane out of range condition for default_async_event_handler.
     2/16/06  v2.12.12r5 Added ISM support and OEM TTL controls
    10/21/05  v2.12.11   Added reset_ptu_pan and reset_ptu_tilt commands.
     ...
     7/ 3/03: v1.09.13   Corrected version checking in function firmware_version_OK
    ...
    11/10/99: v1.09.11   Added asynchronous event status handling functions.
     8/18/99: v1.09.10   Select_unit_ID binary opcode code added and commented
                         for users wishing to use binary unit selects (i.e.,
                                     by uncommenting the binary op and commenting out the ASCII op).
     7/28/99: v1.09.09r2 In firmware_version_OK, made sure it works even when
                                     the controller is in feedback terse mode. Also added
                         10 sec timeout on read to allow fall-out from non-responsive PTU.
    5/12/99: v1.09.09r1  select_unit declarations cleaned up
    2/ 8/99: v1.09.09.   select_unit operation performed using ASCII select to aid
                                     debugging for developers.
    1/26/99: v1.09.08.   select_unit was blocking for a getchar that doesn't happen...
...
#define PTU_CPI_CODE_VERSION   "v1.09.15"
...
unsigned char default_async_event_handler(unsigned char async_event)
{
  ...
  {
    case PAN_LIMIT_HIT:
      printf("PAN_LIMIT_HIT\n\n\n");
      break;
    case TILT_LIMIT_HIT:
      printf("TILT_LIMIT_HIT\n\n\n");
      break;
    case CABLE_DISCONNECT_DETECTED:
      printf("CABLE_DISCONNECT_DETECTED\n\n\n");
      break;
    default:
      //printf("ERROR (unknown async_event signalled)\n\n\n");
      break;
  }
  return (TRUE);
}
...
DPPanTiltUnit::DPPanTiltUnit():
    err(0),
    speed_control_mode(PTU_INDEPENDENT_SPEED_CONTROL_MODE),
    CHA_comm_active(FALSE)
{
  set_async_event_handler( (void(*)(unsigned char)) default_async_event_handler);
}
...
*/
unsigned char DPPanTiltUnit::set_async_event_handler(void(*async_event_handler)(unsigned char))
{
...
portstream_fd DPPanTiltUnit::open_host_port(const char *portname)
{
  char out_string[10] = "    ";
...
extern char DPPanTiltUnit::set_baud_rate(int baudrate)
{
  return setbaudrate(baudrate);
}

/* close_host_port(<portstream>) ==> <status> */
char DPPanTiltUnit::close_host_port(portstream_fd portstream)
{
...
unsigned char DPPanTiltUnit::GetSerialChar(char await_char)
{
  unsigned char c;
...
unsigned char DPPanTiltUnit::get_binary_command_return_status()
{
...
char DPPanTiltUnit::SerialOut(unsigned char outchar)
{
  return SerialBytesOut(current_host_port, &outchar, 1);
}


unsigned char DPPanTiltUnit::SerialIn()
{
  unsigned char Cin;
...
char DPPanTiltUnit::select_host_port(portstream_fd portstream)
{
...
/* reset_PTU() ==> <status> */
char DPPanTiltUnit::reset_ptu(void)
{
  unsigned char c;
...
char DPPanTiltUnit::reset_ptu_pan(void)
{
  unsigned char c;
...
char DPPanTiltUnit::reset_ptu_tilt(void)
{
  unsigned char c;
...
/* updated 7/3/03 */
char DPPanTiltUnit::firmware_version_OK(void)
{
...
char DPPanTiltUnit::reset_PTU_parser(long timeout_in_msec)
{
...
/* set_desired([PAN|TILT],
        ...
        set_desired([PAN|TILT],
                ...
                <power mode>,
                ABSOLUTE) ==> <status>                              */
char DPPanTiltUnit::set_desired(char axis, char kinematic_property, PTU_PARM_PTR *value, char movement_mode)
{
...
char DPPanTiltUnit::await_completion(void)
{
...
/* get_current([PAN|TILT],
                [POSITION|SPEED|ACCELERATION|BASE|
                 ...
                 HOLD_POWER_LEVEL|MOVE_POWER_LEVEL|
         ISM_DRIFT]) ==> <value> */
long DPPanTiltUnit::get_current(char axis, char kinematic_property)
{
  unsigned short int uvalue;
...
long DPPanTiltUnit::get_desired(char axis, char kinematic_property)
{
  unsigned short int uvalue;
  signed short int value;
  long long_value;
...
*/
char DPPanTiltUnit::set_mode(char mode_type, char mode_parameter)
{
  switch (mode_type)
  {
    case DEFAULTS:
...
char DPPanTiltUnit::halt(char halt_type)
{
  switch (halt_type)
  {
    case PAN:
...
char* DPPanTiltUnit::firmware_version(void)
{
  int charsRead;
 ...
 */
char DPPanTiltUnit::select_unit(UID_fd unit_ID)
{
  char UID_select[10];
...
/*  modified 10/19/98  */
char DPPanTiltUnit::set_unit_id(UID_fd unit_ID)
{
...
/* added 5/28/2004 */
char DPPanTiltUnit::set_PTU_motion(short int desired_pan_pos, short int desired_tilt_pos,
                                   unsigned short int desired_pan_speed, unsigned short int desired_tilt_speed)
{
...
/* added 5/28/2004 */
char DPPanTiltUnit::get_PTU_motion(short int *desired_pan_pos, short int *desired_tilt_pos,
                                   unsigned short int *desired_pan_speed, unsigned short int *desired_tilt_speed)
{
  ...
  GetUnsignedShort(current_host_port, desired_pan_speed, 500);
  GetUnsignedShort(current_host_port, desired_tilt_speed, 500);
  return (PTU_OK);
}
...
char DPPanTiltUnit::talkto_PTUcontroller()
{
  char status;
  char commandResponse[256];
  int charsRead;
...
char DPPanTiltUnit::set_pure_velocities(signed short int *pan_speed, signed short int *tilt_speed)
{
...
unsigned char DPPanTiltUnit::set_TTL_outputs(unsigned char in_TTL_control_byte)
{
...
/* Returns byte with: 0 IP4 IP3 IP2 OP5 OP4 OP3 OP2 */
unsigned char DPPanTiltUnit::get_TTL_values()
{
  unsigned char outbyte;
...
/* returns status */
unsigned char DPPanTiltUnit::TriggerOn(signed short int startPanPos, signed short int deltaPos,
...
/* Returns status */
unsigned char DPPanTiltUnit::TriggerOff()
{ /* send the command */
  SerialOut(TRIGGER_CONTROLS);
...
/* Returns status */
signed short int DPPanTiltUnit::TriggersPending()
{
...
char DPPanTiltUnit::set_desired_abs_positions(signed short int *Ppos, signed short int *Tpos)
{ /* send the pan position command C1 */
  ...
  return (get_binary_command_return_status() /* R1 response from command C1 */
  && get_binary_command_return_status() /* R2 response from command C2 */
  );
}

char DPPanTiltUnit::set_desired_positions_velocities(short int* pan_pos, unsigned short int* pan_vel,
                                      short int* tilt_pos, unsigned short int* tilt_vel)
{
  unsigned short int uvalue;

  SerialOut(PAN_SET_ABS_SPEED);
  uvalue = *((unsigned short *)pan_vel);
  PutUnsignedShort(current_host_port, &uvalue);
  SerialOut(PAN_SET_ABS_POS);
  PutSignedShort(current_host_port, pan_pos);

  SerialOut(TILT_SET_ABS_SPEED);
  uvalue = *((unsigned short *)tilt_vel);
  PutUnsignedShort(current_host_port, &uvalue);
  SerialOut(TILT_SET_ABS_POS);
  PutSignedShort(current_host_port, tilt_pos);

  return (get_binary_command_return_status()
      && get_binary_command_return_status()
      && get_binary_command_return_status()
      && get_binary_command_return_status()
  );
...
/* a faster way to query position commands that overlaps command execution */
char DPPanTiltUnit::get_current_positions(signed short int *Ppos, signed short int *Tpos)
{
  char err1, err2;
  ...
  //err1 = getSignedShortCheckAsync(Ppos);
  //err2 = getSignedShortCheckAsync(Tpos);
  //printf("err1 = %d, err2 = %d\n", err1, err2);
  return (err1 && err2);
}

char DPPanTiltUnit::getSignedShortCheckAsync(signed short *val)
{
  unsigned char b1 = SerialIn();
  while (ASYNCHRONOUS_EVENT(b1))
  {
    (*async_event_handler_fn_ptr)(b1);
    b1 = SerialIn();
  }
  unsigned char b2 = SerialIn();
...
char DPPanTiltUnit::set_ISM_desired_abs_positions(signed short int *Ppos, signed short int *Tpos)
{
  /* send the pan position command C1 */
...
char DPPanTiltUnit::get_ISM_desired_abs_positions(signed short int *Ppos, signed short int *Tpos)
{
  char err1, err2;
...
unsigned char DPPanTiltUnit::checksum_on_2_2B(unsigned char *val1, unsigned char *val2)
{
...
unsigned char DPPanTiltUnit::ptu_set_desired_velocities(signed short int pspeed, signed short int tspeed)
{
...
unsigned char DPPanTiltUnit::execute_set_desired_velocities(signed short int *Pspeed, signed short int *Tspeed)
{
...
/*unsigned char DPPanTiltUnit::set_desired_velocities_get_positions(signed short int pan_vel, signed short int tilt_vel,
                                                                  short int* pan_pos, short int* tilt_pos)
{
  unsigned char checksum, status, err1, err2;
  // send the command
  SerialOut(SET_DESIRED_VELOCITES_GET_POS);
  PutSignedShort(current_host_port, &pan_vel);
  PutSignedShort(current_host_port, &tilt_vel);
  checksum = checksum_on_2_2B((unsigned char *)&pan_vel, (unsigned char *)&tilt_vel);
  SerialOut(checksum);
  // get the command respons
  //err1 = GetSignedShort(current_host_port, pan_pos, -1);
  //err2 = GetSignedShort(current_host_port, tilt_pos, -1);
  status = get_binary_command_return_status(); // GetSerialChar(FALSE);
  return (status);
...
*/
/*
bool DPPanTiltUnit::initialize()
{
  char ret = reset_ptu();
  return printErrorCode(ret);
}

bool DPPanTiltUnit::sendVelocities(signed short int pan_speed, signed short int tilt_speed)
{
  char ret = execute_set_desired_velocities(&pan_speed, &tilt_speed);
  return printErrorCode(ret);
}

bool DPPanTiltUnit::getPosVel(short int& pan_pos, short int& tilt_pos, unsigned short int& pan_vel, unsigned short int& tilt_vel)
{
  char ret = get_PTU_motion(&pan_pos, &tilt_pos, &pan_vel, &tilt_vel);
  return printErrorCode(ret);
}

bool DPPanTiltUnit::getPositions(short int& pan_pos, short int& tilt_pos)
{
  char ret = get_current_positions(&pan_pos, &tilt_pos);
  if (ret==1)
    return true;
  else return false;
}

bool DPPanTiltUnit::sendVelGetPos(signed short int pan_speed, signed short int tilt_speed,
                   short int& pan_pos, short int& tilt_pos)
{
  char ret = set_desired_velocities_get_positions(pan_speed, tilt_speed, &pan_pos, &tilt_pos);
  return printErrorCode(ret);
}
*/
}
