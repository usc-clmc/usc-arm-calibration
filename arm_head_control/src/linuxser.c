...
static const int LOG_COMMS=0;
static FILE* comms_log=NULL;
...
static descriptor_t *descriptor_lookup(int fd){
   ...
   return bsearch(&key, descriptors, MAX_PORTS,
         sizeof(descriptor_t), descriptor_compare);
}
...
int serial_init(void){
   ...
   if (LOG_COMMS && comms_log==NULL)
   {
     comms_log=fopen("/tmp/serial_comms.txt","w");
   }
...
int serial_send(int fd, void *buff, int len){
   ...
   bleft = len;
   while((bwrite = write(fd, wptr, bleft)) >= 0 &&
         (bleft - bwrite) > 0){
      bleft -= bwrite;
      wptr  += bwrite;
   }
   ...
   {
     fprintf(comms_log, ">> ");
     fwrite(buff, 1, len-bleft, comms_log);
     fprintf(comms_log, "\n");
     fflush(comms_log);
   }
...
int serial_recv(int fd, void *buff, int len){
   ...
   bleft = len;
   while((bread = read(fd, rptr, bleft)) >= 0 &&
         (bleft - bread) > 0){
      bleft -= bread;
      rptr  += bread;
   }
...
char SerialBytesIn(portstream_fd fd, unsigned char *buffer,
      unsigned int nBytes, long timeoutVal){
   unsigned int bytes_left;
   int          c;

   bytes_left = nBytes;
   while(bytes_left > 0 && serial_can_recv(fd, timeoutVal)){
...
char FlushOutputBuffer(portstream_fd fd){
   tcflush(fd, TCOFLUSH);
   return 1;
}
char FlushIOBuffer(portstream_fd fd){
...
char SerialStringOut(portstream_fd fd, unsigned char *buffer){
   if(serial_send(fd, buffer, strlen((char *)buffer)) != (int)strlen((char *)buffer))
      return -1;
   return 1;
}


char ReadSerialLine(portstream_fd fd, unsigned char *strbuffer, 
      long timeout, int *charsRead){
   *charsRead = 0;
   while(serial_can_recv(fd, timeout)){
...
char GetSignedShort(portstream_fd fd, signed short *SHORTval,
      long timeout){
...
char GetUnsignedShort(portstream_fd fd, unsigned short *USHORTval,
      long timeout){
...
char GetSignedLong(portstream_fd fd, signed long *LONGval,
      long timeout){
