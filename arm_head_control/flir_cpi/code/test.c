...
#define TEST_CODE_VERSION       "v1.09.14"
/****            (C)1992-2007, Directed Perception, Inc.             *****
*****                     All Rights Reserved.                       *****
*****                                                                *****
...
CHANGE HISTORY:  
	 3/19/07   v1.09.12    Added ISM test suite and Commands
    10/21/05   v1.09.11    Added reset_ptu_pan and reset_ptu_tilt test commands. 
    10/03/05   v1.09.10    Changed some parameters to work work with D300              
...
{	switch (status_code) {
     case  PTU_OK:	
		   strcpy(error_string,"PTU_OK"); 		   break;
     case  PTU_ILLEGAL_COMMAND:							
		   strcpy(error_string, "PTU_ILLEGAL_COMMAND"); 		   break;
     case  PTU_ILLEGAL_POSITION_ARGUMENT:							
		   strcpy(error_string, "PTU_ILLEGAL_POSITION_ARGUMENT"); 		   break;
     case  PTU_ILLEGAL_SPEED_ARGUMENT:							
		   strcpy(error_string, "PTU_ILLEGAL_SPEED_ARGUMENT"); 		   break;
     case  PTU_ACCEL_TABLE_EXCEEDED:							
		   strcpy(error_string, "PTU_ACCEL_TABLE_EXCEEDED"); 		   break;
     case  PTU_DEFAULTS_EEPROM_FAULT:							
		   strcpy(error_string, "PTU_DEFAULTS_EEPROM_FAULT"); 		   break;
     case  PTU_SAVED_DEFAULTS_CORRUPTED:							
		   strcpy(error_string, "PTU_SAVED_DEFAULTS_CORRUPTED"); 		   break;
     case  PTU_LIMIT_HIT:							
		   strcpy(error_string, "PTU_LIMIT_HIT"); 		   break;
     case  PTU_CABLE_DISCONNECTED:							
		   strcpy(error_string, "PTU_CABLE_DISCONNECTED"); 		   break;
     case  PTU_ILLEGAL_UNIT_ID:							
		   strcpy(error_string, "PTU_ILLEGAL_UNIT_ID"); 		   break;
     case  PTU_ILLEGAL_POWER_MODE:							
		   strcpy(error_string, "PTU_ILLEGAL_POWER_MODE"); 		   break;
     case  PTU_RESET_FAILED:							
		   strcpy(error_string, "PTU_RESET_FAILED"); 		   break;
     case  PTU_ILLEGAL_PARAMETERS:							
		   strcpy(error_string, "PTU_ILLEGAL_PARAMETERS"); 		   break;
     case  PTU_DUART_ERROR:							
		   strcpy(error_string, "PTU_DUART_ERROR"); 		   break;
     case  PTU_ERROR:							
		   strcpy(error_string, "PTU_ERROR"); 		   break;
     case  NOT_SUPPORTED_BY_THIS_FIRMWARE_VERSION:							
		   strcpy(error_string, "NOT_SUPPORTED_BY_THIS_FIRMWARE_VERSION"); 		   break;
     case  PTU_TILT_VANE_OUT_OF_RANGE_ERROR:							
		   strcpy(error_string, "PTU_TILT_VANE_OUT_OF_RANGE_ERROR"); 		   break;
     default:							
		   strcpy(error_string, "UNDEFINED_RETURN_STATUS_ERROR"); 		   break;
}
}
...
#define TOP_SPEED 8000
	  ...
	  { // Sleep(0);
		...
		{ if (val1==0) zero_count++;
		  if (zero_count >10) return(-1);
		  if (zero_count >3) printf("  (C%d, D%d)", val1, speed);
		  //printf(" (hit key to continue): "); getchar();
		}
	    if ((speed % 100) == 0) printf("\nspeed %d... ", speed);
...
int main()
    ...
    /* testing ISM commands */
	/*{ signed short int i, j=-1000;
   
      set_mode(SPEED_CONTROL_MODE, PTU_PURE_VELOCITY_SPEED_CONTROL_MODE);
	  for (i=0; i<1800; i++)
	  { j = -j;
		if (execute_set_desired_velocities(&j, &j) != PTU_OK)
			printf("\n! checksum error");
		// printf("  %d(%d)  ", j, execute_set_desired_velocities(&j, &j));
	  }
	}
	goto exit;*/
...
/* test_DB15_extensions(COMstream); goto exit; */

    /* testing OEM TTL commands */
/*	{ unsigned char ttl_vout, ttl_vin;
	  for (ttl_vout = 0; ttl_vout < 16; ttl_vout++) {
		  ttl_vin = set_TTL_outputs(ttl_vout);
		  printf("\nset_TTL_outputs(%d) returned %d", ttl_vout, ttl_vin);
		  do_delay(2000);
	  }
	  ttl_vout = 0;
	  ttl_vin = set_TTL_outputs(ttl_vout);
	  printf("\nset_TTL_outputs(%d) returned %d", ttl_vout, ttl_vin);
	  ttl_vin = get_TTL_values();
	  printf("\nget_TTL_values returned %d", ttl_vin);
      printf("\nChange input value enter any character ");  return_error(); 
	  ttl_vin = get_TTL_values();
	  printf("\nget_TTL_values returned %d", ttl_vin);
	  goto exit;
	}	  
...
/*
	if ( config_CHA() ) 
	  if ( talkto_CHA() )
	     {  if ( talkto_PTUcontroller() )
	        printf("\n\nTEST worked!\n\n");
		    else printf("\n\ntalkto_PTUcontroller() failed...\n\n");
	  }
	  else printf("\n\ntalkto_CHA() failed...\n\n");
	else printf("\n\nconfig_CHA() failed...\n\n");
*/

	/* below line tests fix for slow speed moves. Only works for firmware v1.10.1r3 and higher */
	/* if ( test_low_speed_moves() ) goto abnormal_exit;   */


	/* %% test what is returned for await completion when limits are disabled, and the unit is commanded outside limits */
/*	{ short int val=-6000;
	  unsigned char commandString[64]="   LD TP0 I ";
	  
      SerialBytesOut(COMstream, (unsigned char*) commandString, strlen(commandString));
	  do_delay(5000);
	  FlushInputBuffer(COMstream);

      set_desired(TILT,  POSITION, (PTU_PARM_PTR *) &val, ABSOLUTE); 
	  printf("\nawait_completion returned %d\n", await_completion() );
	  return(TRUE);

	}
*/

	/* below tests limit asynchronous detect event */
/*	
	{ int position;

	  lval1 = get_desired(PAN,MINIMUM_POSITION);
	  printf("\n\nPAN_MINIMUM_POSITION_QUERY RETURNED: %ld\n", lval1);
	  lval2 = get_desired(PAN,MAXIMUM_POSITION);
	  printf("PAN_MAXIMUM_POSITION_QUERY RETURNED: %ld\n", lval2);

	  cval = PTU_OFF_POWER;
	  set_desired(PAN, HOLD_POWER_LEVEL, (PTU_PARM_PTR *) &cval, ABSOLUTE);
	  FlushInputBuffer(COMstream);
	  printf("\n\nMotor powered off.\nRotate pan motor knob so pax axis loses its known position");
	  while ( tolower(getchar()) != 'c' ) printf("\n(enter 'c' to continue): ");
         
	  set_desired(PAN, POSITION, (PTU_PARM_PTR *) &lval1, ABSOLUTE); 
      while ( (position=get_current(PAN,POSITION)) != lval1) 
	  { printf("%d ", position); Sleep(100); }

      set_desired(PAN, POSITION, (PTU_PARM_PTR *) &lval2, ABSOLUTE); 
      while ( (position=get_current(PAN,POSITION)) != lval2) 
	  { printf("%d ", position); Sleep(100); } 

	  set_desired(PAN, POSITION, (PTU_PARM_PTR *) &lval1, ABSOLUTE);
	  lval_in = 99999;
	  while ( (lval=get_current(PAN,POSITION)) != lval1) 
	  {   if (lval > lval_in) { printf("\nvalue1 is less! ('%d')", *((unsigned char *) &lval)); }
		  if (lval==lval_in) 
			{ printf("\n1: Limit hit, unit did not reach end position\n");
			  break; 		}
	      else { lval_in = lval;
				 printf("cpos1=%d ", lval); Sleep(1000); }
		}

	  set_desired(PAN, POSITION, (PTU_PARM_PTR *) &lval2, ABSOLUTE);
	  while ( (lval=get_current(PAN,POSITION)) != lval2) 
		{ if (lval > lval_in) { printf("\nvalue2 is less! ('%d')", *((unsigned char *) &lval)); }
		  if (lval==lval_in) 
			{ printf("\n2: Limit hit, unit did not reach end position\n");
			  break; 		}
		  else { lval_in = lval;
				 printf("cpos2=%d ", lval); Sleep(1000); }
		}
...
*/
    ...
    /* below tests a single call to change pan and tilt position and speed. Working with firmware 2.12.9r2 and above */
	/*
    { unsigned char opcode = 222;
	  short int desired_pan_pos, desired_tilt_pos;
	  unsigned short int desired_pan_speed, desired_tilt_speed;
	  
	  printf("\n\n\n");
	  for (;;) {

		  desired_pan_pos   = desired_tilt_pos   = 2000;
		  desired_pan_speed = desired_tilt_speed = 1000;
		  printf("SENT: P(%d, %d) S(%d, %d)\n", desired_pan_pos, desired_tilt_pos, 
						 desired_pan_speed, desired_tilt_speed);
		  set_PTU_motion(desired_pan_pos,   desired_tilt_pos, 
						 desired_pan_speed, desired_tilt_speed);	
		  do_delay(500);
		  get_PTU_motion(&desired_pan_pos,   &desired_tilt_pos, 
						 &desired_pan_speed, &desired_tilt_speed);
		  printf("   READ: P(%d, %d) S(%d, %d)\n", desired_pan_pos,   desired_tilt_pos, 
										  desired_pan_speed, desired_tilt_speed);
		  await_completion();


	      desired_pan_pos   = desired_tilt_pos   = -2000;
		  desired_pan_speed = desired_tilt_speed = 2900;
		  printf("SENT: P(%d, %d) S(%d, %d)\n", desired_pan_pos, desired_tilt_pos, 
						 desired_pan_speed, desired_tilt_speed);
		  set_PTU_motion(desired_pan_pos,   desired_tilt_pos, 
						 desired_pan_speed, desired_tilt_speed);
