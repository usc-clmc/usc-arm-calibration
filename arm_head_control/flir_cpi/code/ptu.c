...
{   trace_printf("\n\n\n\ndefault_async_event_handler called: ");
	switch (async_event) {
	case PAN_LIMIT_HIT:					trace_printf("PAN_LIMIT_HIT\n\n\n");
										break;
	case TILT_LIMIT_HIT:				trace_printf("TILT_LIMIT_HIT\n\n\n");
										break;
	case CABLE_DISCONNECT_DETECTED:		trace_printf("CABLE_DISCONNECT_DETECTED\n\n\n");
										break;
	default:							trace_printf("ERROR (unknown async_event signalled)\n\n\n");
										break;
	}
	return(TRUE);
}
...
{   unsigned char *tmpStr;
	...
	if ( /* ensure the version numbers seem reasonable as version numbers, and
		 ...
		 ( (modelVersion < 999)               && 
		   (modelVersion > PTU_modelVersion))  
		 ||
		 ( (codeVersion  < 999)               && 
		   (modelVersion == PTU_modelVersion) && 
		   (codeVersion  > PTU_codeVersion) )  
		 ||
		 ( (revision     < 9999) && 
		   (modelVersion == PTU_modelVersion) && 
		   (codeVersion  == PTU_codeVersion)  &&
		   (revision     >= PTU_revision)) ) 
		  { trace_printf("\n\nController firmware v%d.%d.%d is compatible\n\n",
			 ...
			 return(TRUE); /* the controller firmware is compatible */
		      ...
		      modelVersion, codeVersion, revision,
		      PTU_modelVersion, PTU_codeVersion, PTU_revision, charsRead);
	       return(FALSE);
	}
}
	...
	{ unsigned short int uvalue;
	  ...
	  switch (kinematic_property)	{
			...
			case ISM_DRIFT:   /*** ISM supported added 16Feb2006 ***/
					switch (axis)  {
						case PAN:  SerialOut(ISM_SET_PAN_DRIFT_RATE);
								   break;
						case TILT: SerialOut(ISM_SET_TILT_DRIFT_RATE);
								   break;
						default: return(PTU_ILLEGAL_COMMAND_ARGUMENT);		}
					PutSignedShort(current_host_port, (signed short *) value);
					break;
	...
	{ unsigned short int uvalue;
	  ...
	  switch (kinematic_property)	{
			...
			case ISM_DRIFT: /*** added ISM support 16Feb2006 ***/
					switch (axis)  {
						case PAN:	SerialOut(ISM_GET_PAN_DRIFT_RATE);
									goto get_and_return_signed_short_int;
						case TILT:  SerialOut(ISM_GET_TILT_DRIFT_RATE);
									goto get_and_return_signed_short_int;
						default: 	return(PTU_ILLEGAL_COMMAND_ARGUMENT);
						}
			case ACCELERATION:
...
char set_mode(char mode_type, char mode_parameter)
  ...
  return_status:
	return( get_binary_command_return_status() );	/* return <status> */
}
...
/* added 5/28/2004 */
char set_PTU_motion(short int desired_pan_pos, short int desired_tilt_pos, 
...
/* added 5/28/2004 */
char get_PTU_motion(short int *desired_pan_pos, short int *desired_tilt_pos, 
					unsigned short int *desired_pan_speed, unsigned short int *desired_tilt_speed)
...
{ char status; 
  char commandResponse[256];
  int charsRead;
  if ( CHA_comm_active != TRUE ) return ( FALSE );
